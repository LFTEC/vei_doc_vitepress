...
<mark>非常重要：使用该方式时，不可使用定时同步库存的方式，否则后续库存同步时有可能造成万华端存储的供应商备货库存量虚高。由此造成万华需求无法满足时，相应的考核扣分由供应商自行承担！</mark>
...
<mark>使用该同步方式时，接收到万华的采购需求必须采用[锁定库存并通知](#锁定库存并通知)的方式，必须将万华专有锁定库存与备库库存相加作为总备库数量进行推送</mark>

$$
物料备库数量 = 备库数量（供应商系统） + 万华锁定数量（万华系统推送的锁定数量）
$$
由于万华锁定数量由万华数据为准，
